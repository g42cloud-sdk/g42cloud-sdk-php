...
class PrePaidServer implements ModelInterface, ArrayAccess
{
    ...
    * imageRef  待创建云服务器的系统镜像，需要指定已创建镜像的ID，ID格式为通用唯一识别码（Universally Unique Identifier，简称UUID）。  镜像的ID可以从镜像服务的 [查询镜像列表] 接口获取，该接口可根据 __imagetype、__os_type 等参数过滤选择合适镜像。
    * flavorRef  待创建云服务器的系统规格的ID。  可通过 [规格列表接口]查询，该接口支持通过 availability_zone 参数过滤出待创建云服务器可用区下可用的规格。  已上线的规格请参见《[弹性云服务器产品介绍](https://support.huaweicloud.com/ecs/index.html)》的“实例类型与规格”章节。
    ...
    * adminPass  如果需要使用密码方式登录云服务器，可使用adminPass字段指定云服务器管理员帐户初始登录密码。其中，Linux管理员帐户为root，Windows管理员帐户为Administrator。具体使用方法请参见背景信息（设置登录鉴权方式）。  密码复杂度要求：   - 长度为8-26位。  - 密码至少必须包含大写字母、小写字母、数字和特殊字符（!@$%^-_=+[{}]:,./?）中的三种。 - 密码不能包含用户名或用户名的逆序。  - Windows系统密码不能包含用户名或用户名的逆序，不能包含用户名中超过两个连续字符的部分。
    ...
    * vpcid  待创建云服务器所属虚拟私有云（简称VPC），需要指定已创建VPC的ID，UUID格式。  可通过 [查询VPC列表] 接口查询。
    ...
    * availabilityZone  待创建云服务器所在的可用分区，需要指定可用分区（AZ）的名称。  可通过接口 [查询可用区列表接口] 获取，也可参考[地区和终端节点]获取。
    ...
    */
    protected static $openAPITypes = [
            ...
            'keyName' => 'string',
            'vpcid' => 'string',
            'nics' => '\G42Cloud\SDK\Ecs\V2\Model\PrePaidServerNic[]',
            'publicip' => '\G42Cloud\SDK\Ecs\V2\Model\PrePaidServerPublicip',
            'count' => 'int',
            'isAutoRename' => 'bool',
            'rootVolume' => '\G42Cloud\SDK\Ecs\V2\Model\PrePaidServerRootVolume',
            'dataVolumes' => '\G42Cloud\SDK\Ecs\V2\Model\PrePaidServerDataVolume[]',
            'securityGroups' => '\G42Cloud\SDK\Ecs\V2\Model\PrePaidServerSecurityGroup[]',
            'availabilityZone' => 'string',
            'batchCreateInMultiAz' => 'bool',
            'extendparam' => '\G42Cloud\SDK\Ecs\V2\Model\PrePaidServerExtendParam',
            'metadata' => 'map[string,string]',
            'osschedulerHints' => '\G42Cloud\SDK\Ecs\V2\Model\PrePaidServerSchedulerHints',
            'tags' => 'string[]',
            'serverTags' => '\G42Cloud\SDK\Ecs\V2\Model\PrePaidServerTag[]',
            'description' => 'string'
    ];
    ...
    */
    ...
    /**
    * Gets imageRef
    *  待创建云服务器的系统镜像，需要指定已创建镜像的ID，ID格式为通用唯一识别码（Universally Unique Identifier，简称UUID）。  镜像的ID可以从镜像服务的 [查询镜像列表] 接口获取，该接口可根据 __imagetype、__os_type 等参数过滤选择合适镜像。
    *
    * @return string
    */
    ...
    * Sets imageRef
    *
    * @param string $imageRef 待创建云服务器的系统镜像，需要指定已创建镜像的ID，ID格式为通用唯一识别码（Universally Unique Identifier，简称UUID）。  镜像的ID可以从镜像服务的 [查询镜像列表] 接口获取，该接口可根据 __imagetype、__os_type 等参数过滤选择合适镜像。
    *
    * @return $this
    */
    ...
    /**
    * Gets flavorRef
    *  待创建云服务器的系统规格的ID。  可通过 [规格列表接口]查询，该接口支持通过 availability_zone 参数过滤出待创建云服务器可用区下可用的规格。  已上线的规格请参见《[弹性云服务器产品介绍](https://support.huaweicloud.com/ecs/index.html)》的“实例类型与规格”章节。
    *
    * @return string
    */
    ...
    * Sets flavorRef
    *
    * @param string $flavorRef 待创建云服务器的系统规格的ID。  可通过 [规格列表接口]查询，该接口支持通过 availability_zone 参数过滤出待创建云服务器可用区下可用的规格。  已上线的规格请参见《[弹性云服务器产品介绍](https://support.huaweicloud.com/ecs/index.html)》的“实例类型与规格”章节。
    *
    * @return $this
    */
    ...
    */
    ...
    /**
    * Gets vpcid
    *  待创建云服务器所属虚拟私有云（简称VPC），需要指定已创建VPC的ID，UUID格式。  可通过 [查询VPC列表] 接口查询。
    *
    * @return string
    */
    ...
    * Sets vpcid
    *
    * @param string $vpcid 待创建云服务器所属虚拟私有云（简称VPC），需要指定已创建VPC的ID，UUID格式。  可通过 [查询VPC列表] 接口查询。
    *
    * @return $this
    */
    ...
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerNic[]
    */
    ...
    * Sets nics
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerNic[] $nics 待创建云服务器的网卡信息。  约束：  - 网卡对应的子网（subnet）必须属于vpcid对应的VPC。 - 当前单个云服务器支持最多挂载12张网卡。 - 不同的规格对网卡上限有一定的区别，参考[规格清单](https://support.huaweicloud.com/productdesc-ecs/zh-cn_topic_0159822360.html)。
    *
    * @return $this
    */
    ...
    *  publicip
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerPublicip|null
    */
    ...
    * Sets publicip
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerPublicip|null $publicip publicip
    *
    * @return $this
    */
    ...
    *  rootVolume
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerRootVolume
    */
    ...
    * Sets rootVolume
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerRootVolume $rootVolume rootVolume
    *
    * @return $this
    */
    ...
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerDataVolume[]|null
    */
    ...
    * Sets dataVolumes
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerDataVolume[]|null $dataVolumes 云服务器对应数据盘相关配置。每一个数据结构代表一块待创建的数据盘。   约束：目前新创建的弹性云服务器最多可挂载23块数据盘。
    *
    * @return $this
    */
    ...
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerSecurityGroup[]|null
    */
    ...
    * Sets securityGroups
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerSecurityGroup[]|null $securityGroups 云服务器对应安全组信息。  约束：当该值指定为空时，默认给云服务器绑定default安全组。
    *
    * @return $this
    */
    ...
    *  待创建云服务器所在的可用分区，需要指定可用分区（AZ）的名称。  可通过接口 [查询可用区列表接口] 获取，也可参考[地区和终端节点]获取。
    *
    * @return string|null
    */
    ...
    *
    * @param string|null $availabilityZone 待创建云服务器所在的可用分区，需要指定可用分区（AZ）的名称。  可通过接口 [查询可用区列表接口] 获取，也可参考[地区和终端节点]获取。
    *
    * @return $this
    */
    ...
    *  extendparam
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerExtendParam|null
    */
    ...
    * Sets extendparam
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerExtendParam|null $extendparam extendparam
    *
    * @return $this
    */
    ...
    *  osschedulerHints
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerSchedulerHints|null
    */
    ...
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerSchedulerHints|null $osschedulerHints osschedulerHints
    *
    * @return $this
    */
    ...
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerTag[]|null
    */
    ...
    * Sets serverTags
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\PrePaidServerTag[]|null $serverTags 弹性云服务器的标签。  > 说明： >  > 创建弹性云服务器时，一台弹性云服务器最多可以添加10个标签。 > 公有云新增server_tags字段，该字段与tags字段功能相同，支持的key、value取值范围更广，建议使用server_tags字段。
    *
    * @return $this
    */
