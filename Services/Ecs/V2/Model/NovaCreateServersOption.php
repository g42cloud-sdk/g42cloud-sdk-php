...
class NovaCreateServersOption implements ModelInterface, ArrayAccess
{
    ...
    * metadata  用户自定义字段键值对。  > - key的长度大于0小于256字节 > - value的长度大于等于0小于256字节   系统预留字段  1. admin_pass：弹性云服务器密码        Windows弹性云服务器Administrator用户的密码。     > 说明：     > 创建密码方式鉴权的Windows弹性云服务器时为必选字段。
    * adminPass  如果需要使用密码方式登录云服务器，可使用adminPass字段指定云服务器管理员帐户初始登录密码。其中，Linux管理员帐户为root，Windows管理员帐户为Administrator。
    * blockDeviceMappingV2  扩展属性，指定弹性云服务器存储设备的v2接口。是存储资源的新版本接口，指定卷场景不能批创弹性云服务器。  裸金属服务器场景不支持。
    * configDrive  扩展属性，开启后在弹性云服务器创建时挂载config_drive向弹性云服务器内部传递信息。  当前不支持该功能。
    * securityGroups  扩展属性，指定弹性云服务器的安全组，默认为default。  指定network创建弹性云服务器时该字段有效。对于已存在端口，安全组请求无效。
    * networks  扩展属性，指定弹性云服务器的网卡信息。有多个租户网络时必须指定。
    * keyName  扩展属性，指定keypair的名称。
    * userData  扩展属性，字符串长度小于65535，且必须是base64加密的。
    * availabilityZone  扩展属性，指定弹性云服务器所在的AZ。  创建弹性云服务器时需要填入该参数。
    * returnReservationId  扩展属性，是否支持返回批量创建弹性云服务器的reservation_id。通过返回的reservation_id，可以过滤查询到本次创建的弹性云服务器。  - true，返回reservation_id。 - false，返回弹性云服务器信息。  > 说明： >  > 批量创建弹性云服务器时，支持使用该字段。
    * minCount  扩展属性，表示创建弹性云服务器最小数量。  默认值为1。  > 说明： >  > 指定镜像创建弹性云服务器时，支持使用该字段。
    * maxCount  表示创建弹性云服务器最大数量。  默认值与min_count的取值一致。  约束：  参数max_count的取值必须大于参数min_count的取值。  当min_count、max_count同时设置时，创弹性云服务器的数量取决于服务器的资源情况。根据资源情况，在min_count至max_count的取值范围内创建最大数量的弹性云服务器。  - 说明： -  - 指定镜像创建弹性云服务器时，支持使用该字段。
    * osDcFdiskConfig  diskConfig的方式，取值为AUTO、MANUAL。  - MANUAL，镜像空间不会扩展。 - AUTO，系统盘镜像空间会自动扩展为与flavor大小一致。  当前不支持该功能。
    * description  扩展属性，表示弹性云服务器描述信息，默认为空字符串。  - 长度最多允许85个字符。 - 不能包含“<” 和 “>”等特殊符号。  > 说明： >  > - V2接口不支持该字段。 > - V2.1接口支持该字段，此时，需在请求Header中增加一组Key-Value值。其中，Key固定为“X-OpenStack-Nova-API-Version” ，Value为微版本号，当Value的值为2.19时，支持使用该字段。
    *
    * @var string[]
    */
    protected static $openAPITypes = [
            ...
            'adminPass' => 'string',
            'blockDeviceMappingV2' => '\G42Cloud\SDK\Ecs\V2\Model\NovaServerBlockDeviceMapping[]',
            'configDrive' => 'string',
            'securityGroups' => '\G42Cloud\SDK\Ecs\V2\Model\NovaServerSecurityGroup[]',
            'networks' => '\G42Cloud\SDK\Ecs\V2\Model\NovaServerNetwork[]',
            'keyName' => 'string',
            'userData' => 'string',
    ...
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\NovaServerBlockDeviceMapping[]|null
    */
    ...
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\NovaServerBlockDeviceMapping[]|null $blockDeviceMappingV2 扩展属性，指定弹性云服务器存储设备的v2接口。是存储资源的新版本接口，指定卷场景不能批创弹性云服务器。  裸金属服务器场景不支持。
    *
    * @return $this
    */
    ...
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\NovaServerSecurityGroup[]|null
    */
    ...
    * Sets securityGroups
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\NovaServerSecurityGroup[]|null $securityGroups 扩展属性，指定弹性云服务器的安全组，默认为default。  指定network创建弹性云服务器时该字段有效。对于已存在端口，安全组请求无效。
    *
    * @return $this
    */
    ...
    *
    * @return \G42Cloud\SDK\Ecs\V2\Model\NovaServerNetwork[]
    */
    ...
    * Sets networks
    *
    * @param \G42Cloud\SDK\Ecs\V2\Model\NovaServerNetwork[] $networks 扩展属性，指定弹性云服务器的网卡信息。有多个租户网络时必须指定。
    *
    * @return $this
    */
