...
class VolumeDetail implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'VolumeDetail';
    ...
    */
    protected static $openAPITypes = [
            'id' => 'string',
            'links' => '\G42Cloud\SDK\Evs\V2\Model\Link[]',
            'name' => 'string',
            'status' => 'string',
            'attachments' => '\G42Cloud\SDK\Evs\V2\Model\Attachment[]',
    ...
    *
    * @return \G42Cloud\SDK\Evs\V2\Model\Link[]
    */
    ...
    * Sets links
    *
    * @param \G42Cloud\SDK\Evs\V2\Model\Link[] $links 云硬盘URI自描述信息。请参见 [links参数说明](https://support.huaweicloud.com/api-evs/evs_04_2006.html#evs_04_2006__evs_04_2010_li1077125119136)。
    *
    * @return $this
    */
    ...
    *
    * @return \G42Cloud\SDK\Evs\V2\Model\Attachment[]
    */
    ...
    * Sets attachments
    *
    * @param \G42Cloud\SDK\Evs\V2\Model\Attachment[] $attachments 云硬盘的挂载信息，请参见•[attachments参数说明](https://support.huaweicloud.com/api-evs/evs_04_2006.html#evs_04_2006__evs_04_2010_li12430153610291)。
    *
    * @return $this
    */
    ...
    * Sets metadata
    *
    * @param map[string,object] $metadata 云硬盘的元数据。  __system__cmkid metadata中的加密cmkid字段，与__system__encrypted配合表示需要加密，cmkid长度固定为36个字节。 >  > 请求获取密钥ID的方法请参考：\"[查询密钥列表](https://support.huaweicloud.com/api-dew/ListKeys.html)\"。  __system__encrypted metadata中的表示加密功能的字段，0代表不加密，1代表加密。 不指定该字段时，云硬盘的加密属性与数据源保持一致，如果不是从数据源创建的场景，则默认不加密。  full_clone 从快照创建云硬盘时的创建方式。 * 0表示使用链接克隆方式。 * 1表示使用全量克隆方式。  hw:passthrough * true表示云硬盘的设备类型为SCSI类型，即允许ECS操作系统直接访问底层存储介质。支持SCSI锁命令。 * false表示云硬盘的设备类型为VBD (虚拟块存储设备 , Virtual Block Device)类型，即为默认类型，VBD只能支持简单的SCSI读写命令。 * 该字段不存在时，云硬盘默认为VBD类型。  orderID metadata中的表示云硬盘计费类型的字段。 当该字段有值时，表示该云硬盘的计费类型为包周期计费，否则计费类型为按需计费。
    *
    * @return $this
    */
    ...
    * Sets shareable
    *
    * @param string $shareable 是否为共享云硬盘。true为共享盘，false为普通云硬盘。 该字段已经废弃，请使用multiattach。
    *
    * @return $this
    */
